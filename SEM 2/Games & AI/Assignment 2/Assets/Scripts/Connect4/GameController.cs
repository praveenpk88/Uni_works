	...
	public class GameController : MonoBehaviour 
	{
		...
		void UpdatePlayAgainButton()
		{
			...
			if (Physics.Raycast(ray, out hit) && hit.collider.name == btnPlayAgain.name)
			{
				...
				// || (Input.touchCount > 0 REMOVED
				if (ClickMoveAction.WasPressedThisFrame() && btnPlayAgainTouching == false)
				{
			...
			// OUTDATED code was used for mobile device
		...
		void Update() 
		{
			...
			if (agents[state.GetPlayerTurn()] == null)
			{
				...
				else
				{
					...
					if(ClickMoveAction.WasPressedThisFrame() && !mouseButtonPressed && !isDropping)
					{
